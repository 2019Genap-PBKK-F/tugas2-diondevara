...
app.delete("/api/jenis-satker/:id", function(req, res)
{
  ...
  var query = "delete from jenis-satker where id = @id" 
  executeQuery(res, query, model, 1)
})
...
app.post("/api/master-indikator/", function(req, res)
{
  ...
  var query = "insert into MasterIndikator ( id_pembilang, id_penyebut, nama, deskripsi, default_bobot, create_date, last_update, expired_date ) values ( @id_penyebut, @id_pembilang, @nama, @deskripsi, @default_bobot, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, @expired_date)"
  executeQuery(res, query, model, 1)
})
...
app.put("/api/master-indikator/:id", function(req, res)
{
  ...
  var query = "update MasterIndikator set id_pembilang = @id_pembilang, id_penyebut = @id_penyebut, nama = @nama, deskripsi = @deskripsi, "
...
{
  var model = [
    { name: 'id_master', sqltype: sql.Int, value: req.body.id },
    { name: 'id_periode', sqltype: sql.Int, value: req.body.id2 }
  ]
...
app.post("/api/satuan-kerja/", function(req, res)
{
  ...
  var query = "insert into SatuanKerja values( @id_ins_satker, @id_induk_satker, @nama, @email, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, @expired_date)"
  executeQuery(res, query, model, 1)
})
...
app.put("/api/satuan-kerja/:id", function(req, res)
{
  ...
  var query = "update SatuanKerja set id_ins_satker = @id_ins_satker, id_induk_satker = @id_induk_satker, nama = @nama, email = @email, last_update = CURRENT_TIMESTAMP " +
              "where id = @id"
  executeQuery(res, query, model, 1)
})
...
app.put("/api/indikator-satuan-kerja/:id&:id2&:id3", function(req, res)
{
  ...
  var query = "update Indikator_SatuanKerja set id_periode = @id_periode, id_master = @id_master, id_satker = @id_satker, bobot = @bobot, target = @target, capaian = @capaian, last_update = CURRENT_TIMESTAMP where id_periode = @id and id_master = @id2 and id_satker = @id3"
  executeQuery(res, query, model, 1)
})
...
app.get("/api/indikator-satuan-kerja-log/", function(req, res){
